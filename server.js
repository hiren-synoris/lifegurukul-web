...
const transporter = nodemailer.createTransport({
    ...
    service: process.env.MAIL_MAILER, // e.g., 'gmail'
    auth: {
...
function getCurrentDateTime() {
    ...
    let month = String(currentDate.getMonth() + 1).padStart(2, '0'); // Adding 1 to month as it's zero-based
...
io.on("connection", (socket) => {
    ...
    socket.on("timerData", async (datas) => {
        var { timer, learner_id_timer, chapter_id_timer, course_id_timer,buy_sell_course_id } =
            datas;
        let Timerconnection;
        try {
            ...
            const course_timer_select = [learner_id_timer, chapter_id_timer, course_id_timer];
            ...
            if ((course_result.length === 0)) {
                ...
                const course_timer_update = [timer, learner_id_timer, chapter_id_timer, course_id_timer];
