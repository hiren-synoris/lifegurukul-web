                    ...
                    <div class="table-responsive">
                                    ...
                                    @if ($valid_till != '')
                                        <tr>
                                            ...
                                            @if ($value->plan_type == \App\Models\CoursePlan::PLAN_FREE)
                                                <td><label for="checked_for_{{$value->id  }}">{{ $value->plan_name ?? '' }} (FREE)
                                                    <br />
                                            ...
                                            @elseif ($value->plan_type == \App\Models\CoursePlan::PLAN_RECURRING)
                                                @if (!empty(config('settings.razorpay_status')))
                                                    <td><label for="checked_for_{{$value->id  }}">{{ $value->plan_name ?? '' }}
                                                        <br />
                                                        @php
                                                            $bill_learner_every =
                        ...
                        @if ($coupons->count() > 0)
                            <div class="coupon_remove">
                                ...
                                <span class="coupon_msg"> </span> <a href="javascript:void(0)" class="cancel_coupon"  style="color:red;display:none;"  title="Cancel coupon"><i class="fa fa-times mt-3" style="font-size: 20px;
                                    padding-left: 4px";="true"></i>
                                </a>
                            </div>
                        @endif
                        @if ($total > 0)
            ...
            <div class="modal-body">
                ...
                @foreach ($coupons as $coupons_val)
                    <div class="apply-inner" style="align-items: center;">
                        ...
                        <div class="apply-heading">
                            <a href="javascript:void(0)" data-coupon_name="{{  $coupons_val->code }}" data-type={{ $coupons_val->type }}
                                data-get_percentage="{{ $coupons_val->value }}"
...
<script>
    $(document).ready(function() {
        ...
        var per_coin_price = "{{ config()->has('settings.onecoinprice') ? config('settings.onecoinprice') : null }}"
        //   if(coupon_count == 0) {
        localStorage.clear()
        //   }
        ...
        $(document).on("click", ".cancel_coupon", function() {
            ...
            var applied_coin = $(".updated_coin").data("applied_coin")
            // var applied_coin
            var available_coin
            var final_price ="";
            var minus_coin
            ...
            if(wallet_value) {
                ...
                if(price > (get_coin*per_coin_price)) {
                    ...
                    available_coin = (minus_coin/per_coin_price)
            ...
            //  updateButton(0, 0, "");
            //  $(".updated_coin").text(" Wallet (Available Coins: " + get_coin + ') (' +
            //                 2 + ' ₹ /- Per Coin)')
            // $(".coin_manage li:nth-child(3)").text("Used Coins: ₹" + '0.00').hide()
            $(".coin_manage li:nth-child(2)").text("Coupon Discount: - ₹" + '0.00').hide()
            ...
            $(".updated_coin").text(" Wallet (Available Coins: " + available_coin+ ' ₹ /- Per Coin)')

            $(".coin_manage li:nth-child(3)").text("Used Coins: ₹" + used_coins)
            $(".coin_manage li:nth-child(1)").text("Price: ₹" + parseFloat(price).toFixed(2));
            $(".coin_manage li:nth-child(4)").text("Final Price: ₹" + parseFloat(final_price).toFixed(2));
            if (price == 0.00) {
        ...
        $(".apply_coupon").click(function() {
            ...
            $(".coupon_msg").text(coupon_name+ " Coupon has been applied").css("color","green")
            ...
            $(".coin_manage li:nth-child(2)").text("Coupon Discount: - ₹" + parseInt(discount) + '.00').show();
            ...
            if ($.trim(applied_coin) !== '' && applied_coin !== '0') {
                ...
                if (applied_coin > price) {
                    ...
                    if (price > 0) {
                        ...
                        $(".coin_manage li:nth-child(3)").text("Used Coins: - ₹" + price + '.' +
                            '00').show();
            ...
            if (checkedValue == 0.00) {
                ...
                if ((parseFloat(coins) * $(".wallet").data("per_price")) > parseFloat(checkedValue)) {
                    ...
                    if ($.trim(get_discount) !== '' && get_discount !== '0') {
                        ...
                        var updated_coin = Math.floor(((applied_coin * per_coin_price) - coins) / per_coin_price)
                        $(".updated_coin").text(" Wallet (Available Coins: " + updated_coin + ') (' +
                        per_coin_price + ' ₹ /- Per Coin)')
                    }
                    ...
                    if ($.trim(get_discount) !== '' && get_discount !== '0') {
                        ...
                        if (parseFloat(final_discount) > (parseFloat(coins) * $(".wallet").data("per_price"))) {
                            var final_totalplus = parseFloat(final_discount) - (parseFloat(coins) * $(".wallet")
                                .data("per_price"))
                ...
                if (coins != 0) {
                    ...
                    $(".coin_manage li:nth-child(3)").text("Used Coins: - ₹" + coins + '.' + '00').show();
                }
                // else {
