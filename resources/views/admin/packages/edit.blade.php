        ...
        <div class="col-12 bg-design">
            ...
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li class="nav-item">
                    ...
                    aria-controls="details" aria-selected="{{($errors->any() && $errors->course_page_title) ? 'false':'true'}}">Details</a> --}}

                    <a class="nav-link active {{ $errors->any()==true ? "active" : "active"}}" id="details-tab" data-toggle="tab" href="#details" role="tab"
                ...
                <li class="nav-item">
                    <a class="nav-link" id="pricing-tab" data-toggle="tab" href="#pricing" role="tab"
                        aria-controls="pricing" aria-selected="{{($errors->any() && $errors->course_page_title) ? 'false':'true'}}">Pricing</a>
                </li>

                <li class="nav-item">
                    {{-- <a class="nav-link {{($errors->any() && $errors->course_page_title) ? 'active':''}}" id="seo-tab" data-toggle="tab" href="#seo" role="tab" aria-controls="seo"
                        aria-selected="{{($errors->any() && $errors->course_page_title) ? 'true':'false'}}">SEO</a> --}}
            ...
            <div class="row justify-content-center mt-5">
                <div class="col-12">
                    <div class="tab-content" id="myTabContent">
                        <!-- Details Tab -->
                        {{-- <div class="tab-pane fade {{($errors->any() && $errors->course_page_title) ? '':'show active'}}" id="details" role="tabpanel" aria-labelledby="details-tab"> --}}
                        <div class="tab-pane fade show active" id="details" role="tabpanel" aria-labelledby="details-tab">

                            {{-- @includeIf('admin.layouts.partials.errors.val  idation-failed') --}}

                            @if (isset($course) && !empty($course) && !empty($course->id))
                                <form method="POST" action="{{ route('packages.update', ['package' => $course->id]) }}"  enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')

                                    <input type="hidden" name="course_id" id="course_id" value="{{ $course->id }}">
                                    <input class="form-check-input select-type" type="hidden"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="order" id="order"
                                            ...
                                            oninput="restrictToTwoDigits(event)"
                                            onclick="this.select()">
                                    </div>
                                ...
                                @if (isset($courseCategories) && !empty($courseCategories) && count($courseCategories) > 0)
                                    <div class="form-group">
                                        <label for=category_id>Package Categories</label><span
                                        ...
                                        <select name="category_id[]" class="form-control course-categories"
                                            ...
                                            {{ $value->name }}
                                                ...
                                                <option @if (in_array($value->id, $arr)) selected @endif
                                                    value="{{ $value->id }}" {{in_array($value->id, old("category_id") ?: []) ? "selected": ""}}>
                                                    {{ $value->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                @endif
                                    ...
                                    <div class="form-group">
                                        <label for="banner_image">Banner Image<small class="text-gray"> Recommended Size ( 1920px * 300px )</small></label>
                                        ...
                                        <div class="image-upload-container">
                                            ...
                                            <a href="{{getImageIfExists($course->banner_image,URL::asset('/front/img/user-love.jpg'))}}">
                                                <img class="img-fluid" style="height: 120px;width: 120px;" alt=""
                                                src="{{getImageIfExists($course->banner_image,URL::asset('/front/img/user-love.jpg'))}}">
                                            </a>
                                                <button class="remove-image" type="button"><i class="fas fa-trash-alt"></i></button>
                                            @endif
                                            <label class="block form-control">
                                                <span class="sr-only">Choose File</span>
                                                <input type="file" name="banner_image" id="intro_video" accept="image/*" />
                                    ...
                                    <div class="form-group">
                                        <label for="intro_video">Introduction Video<small class="text-gray"> Recommended Video type ( mp4 )</small></label>
                                        <label class="block form-control">
                                            <span class="sr-only">Choose File</span>
                                            <input type="file" name="intro_video" id="intro_videos" accept="video/mp4,video/x-m4v,video/*" />
                                        </label>

                                        @if (!empty($course->intro_video))
                                            <video class="video-thumbnail" width="40%" height="auto" controls="controls">
                                            ...
                                            <div class="remove-video">
                                                <button class="remove-video-button" type="button"><i class="fas fa-trash-alt"></i></button>
                                                <input type="hidden" name="remove_intro_video" id="remove_intro_video" value="0">
                                            </div>
                                        @endif
                                        @error('intro_video')
                                            <div class="text text-danger">{{ $message }}</div>
                                        @enderror
                                        <p class="success_msg text-success" style="display:none ">aasfadfaskdhaksjh</p>
                                    ...
                                    <div class="form-group">
                                        <label for=how-to-use>Package Tag line</label>
                                    ...
                                    <label for="course_duration">Package Duration</label>
                                    <div class="form-row">
                                                ...
                                                <div class="card-body"> --}}
                                                    ...
                                                    {{-- <div class="form-group">
                                                        @include('admin.layouts.partials.buttons.toggle-button',[
                                                            ...
                                                            'class' => 'd-inline',
                                                            'toggleBtnText' => 'Allow offline usage on Mobile Apps'
                                                        ])
                                                    </div> --}}
                                    ...
                                    @endif
                                    <div class="form-group">
                                        <label for="">Package can be accessed through</label>
                                        <div class="form-group row p-0 m-0 py-2">
                                            @php
                                                $coursePlatformArray = [];
                                                if (isset($course)) {
                        ...
                        {{-- <div class="tab-pane fade {{($errors->any() && $errors->course_page_title) ? 'show active':''}}" id="seo"
                            role="tabpanel" aria-labelledby="seo-tab"> --}}
                        <div class="tab-pane fade {{ $errors->any() && $errors->course_page_title ? '' : '' }}" id="seo"
                            role="tabpanel" aria-labelledby="seo-tab">
                            @if (isset($course) && !empty($course) && !empty($course->id))
                                <form method="POST" action="{{ route('package.seo.update', ['id' => $course->id]) }}">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div class="form-group">
                                        ...
                                        @php if(!empty($course->meta_keywords)) $keywords = explode(',',$course->meta_keywords); @endphp
                                        <select class="form-control metaKeywords" name="meta_keywords[]"
                                            ...
                                            @if(!empty($course->meta_keywords) && isset($keywords) && count($keywords)>0)
                                                @foreach($keywords as $keywordkey => $keywordvalue)
                                                    <option value="{{$keywordvalue}}" selected>{{$keywordvalue}}</option>
                                                @endforeach
                                            @endif
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea name="meta_description" cols="57" rows="5" id="meta_description"
...
@section('scripts')
    ...
    <script type="text/javascript">
        let browseFile = $('#intro_videos');
        let resumable = new Resumable({
            target: '{{ route('intro_video') }}',
            query: {
                _token: '{{ csrf_token() }}',
                course_id:"{{ request()->route('package') }}"
            },
            fileType: ['mp4'],
