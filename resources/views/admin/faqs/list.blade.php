...
@section('right-section')
    ...
    @can('add_faq', $page)
        @includeIf('admin.layouts.partials.buttons.add',[
            'addUrl' => url('backoffice/faq/create'),
        ])
    @endcan
...
<div style="display: none;" id="blk_del_frm">
        @includeIf('admin.layouts.partials.actions.bulk-delete-data',[
            'bulkDelURL' => url('backoffice/faq/bulk_del')
        ])
        @includeIf('admin.layouts.partials.actions.bulk-hard-delete-data',[
            'bulkHardDelURL' => url('backoffice/faq/bulk_hard_del')
        ])
        @includeIf('admin.layouts.partials.actions.restore-all-data', [
            'restoreAllURL' => url('backoffice/faq/restore_all')
        ])
    </div>
<table id="tbl_faqs" class="table table-bordered table-hove w-100 only_active">
...
@section('scripts')
    ...
    $(document).ready(function() {
        ...
        table = $('#tbl_faqs').DataTable({
            processing: true,
            serverSide: true,
            responsive: true,
            ajax: "{{ url('backoffice/get_faqs') }}",
            columnDefs: [{
        ...
        $('#swWeather').on('switchChange.bootstrapSwitch', function(event, state) {
            ...
            if (state == true) {
                ...
                $('#tbl_faqs').removeClass('only_deleted');
                $('#tbl_faqs').addClass('only_active');
            } else {
                table.destroy();
                table = $('#tbl_faqs').DataTable({
                    processing: true,
                    serverSide: true,
                    responsive: true,
                    ajax: "{{ url('backoffice/get_faqs_deleted') }}",
                    columnDefs: [{
                        className: 'text-center',
                        targets: '_all'
                    }],
                ...
                $('#tbl_faqs').removeClass('only_active');
                $('#tbl_faqs').addClass('only_deleted');
            }
        });
    });
    </script>
