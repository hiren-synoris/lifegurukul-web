...
class V2SettingController extends Controller
{
    /**
    ...
    public function settings(Request $request)
    {
        ...
        $validator_header_data = Validator::make($header_data, [
            'settingtoken' => 'required|filled'
        ]);
        if ($validator_header_data->fails()) {
            $data = Helper::apiResonse(0, $validator_header_data->messages(), []);
            return response()->json($data, 400);
        }
        if($request->header('settingtoken') != "3kSJSGjOqgWud4mLIOOPuXxAe7TrIFMfB4kQPzXAmzNFoVaytFloYVBW0SbpPGej") {
            $data = Helper::apiResonse(0, 'Invaid setting token', []);
            return response()->json($data, 400);
        }
        $keys = [ 'contact_no', 'whatsapp_contact_no','whatsappquerymsg',"igst","sgst","cgst", "iosappforceupdateversion", "androidappforceupdateversion", "hidewelcomescreen"];
        $settings = array();
