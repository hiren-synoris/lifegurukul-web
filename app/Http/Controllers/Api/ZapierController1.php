...
class ZapierController extends Controller
{
    ...
    public function zapierWithLearnerCreate(Request $request)
    {
        ...
        $result = separateCountryCodeAndNumber($fullNumber);
        // if ($result==null) {
        //     return response()->json(["msg" => "Please valid country code for example +919664957352"]);
        // }

        $country_code = @DB::table('countries')->where("phonecode", $result['code'])->first();
        $learner_exist = @Learner::where('mobile', $fullNumber)->where("country_id", $country_code->id)->first();
        if ($learner_exist) {
    ...
    public function zapierWithLearnerEnroll(Request $request)
    {
        ...
        //     return response()->json(["msg" => "Please valid country code for example +919664957352"]);
        // }
        $contry_code = DB::table('countries')->where("phonecode", $request->country_code)->first();

        $plan = CoursePlan::where("id", $request->plan_id)->first();
        if (!$plan) {
            return response()->json(["Msg" => "Plan id not found"]);
        }
        $learner = Learner::where("mobile", $fullNumber)->where("country_id", $contry_code->id)->first();
    ...
    public function zapierEnrollFromPayment(Request $request)
    {
        ...
        $contry_code = DB::table('countries')->where("phonecode", $result["code"])->first();

        $plan = CoursePlan::where("id", $request->plan_id)->first();
        if (!$plan) {
            return response()->json(["Msg" => "Plan id not found"]);
        }
        $learner = Learner::where("mobile", $result["mobile"])->where("country_id", $contry_code->id)->first();
    ...
    public function zapierWithLearnerUnEnroll(Request $request)
    {
        ...
        // if ($result==null) {
        //     return response()->json(["msg" => "Please valid country code for example 9664957352"]);
        // }

        $country_code = DB::table('countries')->where("phonecode", $request->country_code)->first();
        ...
        $learner = Learner::where("mobile", $fullNumber)->where("country_id", $country_code->id)->first();
        ...
        if ($update) {
            ...
            return response()->json(["Msg" => "Something want wrong"]);
        }
    }
}
